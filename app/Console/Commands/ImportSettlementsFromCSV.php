                ...
                try {
                    ...
                    if ($cityName) {
                        ...
                        if (isset($citiesCache[$cityKey])) {
                            ...
                            $city = Locality::where('region_id', $region->id)
                                ->where('name', $cityName)
                                ->first();
                            if (!$city) {
                                // создадим базовый город, если отсутствует
                                $city = Locality::create([
                                    'region_id' => $region->id,
                                    'name' => $cityName,
                                    'slug' => $this->uniqueSlug(Locality::class, Str::slug($cityName)),
                                    'is_active' => true,
                                ]);
                            }
                    ...
                    // Дубликат?
                    $existsQuery = Locality::query()->where('name', $name);
                    if ($city) {
                        $existsQuery->where('locality_id', $city->id);
                    } else {
                        $existsQuery->where('region_id', $region->id)->whereNull('locality_id');
                    }
                    ...
                    Locality::create([
                        ...
                        'locality_id' => $city?->id,
                        'name' => $name,
                        'slug' => $this->uniqueSlug(Locality::class, Str::slug($name)),
                        'type' => $type ?: null,
                        'latitude' => $latitude,
                        'longitude' => $longitude,
                        'population' => null,
