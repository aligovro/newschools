...
class MigrateAllWidgetsToTables extends Command
{
  ...
  public function handle()
  {
    ...
    $query = OrganizationSite::whereNotNull('widgets_config');
    ...
    foreach ($sites as $site) {
      ...
      foreach ($widgetsConfig as $widgetData) {
        try {
          ...
          $widget = Widget::where('slug', $widgetData['slug'])->first();
          $position = WidgetPosition::where('slug', $widgetData['position_slug'])->first();

          if (!$widget) {
            $this->warn("    ⚠️  Widget не найден: {$widgetData['slug']}");
            continue;
          }
    ...
    if ($errors === 0) {
      ...
      $this->info("💡 Теперь можно удалить поле widgets_config из таблицы organization_sites");
    }
