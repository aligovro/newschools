...
class FillDonationsCityId extends Command
{
  ...
  protected $description = 'Заполнить region_id и locality_id в существующих донатах на основе организации';

  public function handle()
  {
    $this->info('Начинаем заполнение region_id и locality_id из организаций...');
    ...
    // Массовое обновление locality_id через SQL
    $updatedCity = DB::affectingStatement("
            UPDATE donations
            INNER JOIN organizations ON organizations.id = donations.organization_id
            SET donations.locality_id = organizations.locality_id
            WHERE donations.locality_id IS NULL
                AND donations.organization_id IS NOT NULL
                AND organizations.locality_id IS NOT NULL
        ");

    $this->info("Обновлено locality_id из организаций: {$updatedCity}");

    // Для оставшихся донатов (где у организации нет locality_id) используем fallback по региону
    $count = DB::table('donations')
      ->whereNull('donations.locality_id')
      ->whereNotNull('donations.organization_id')
      ->join('organizations', 'organizations.id', '=', 'donations.organization_id')
      ->whereNull('organizations.locality_id')
    ...
    if ($count > 0) {
      ...
      $donations = Donation::whereNull('locality_id')
      ...
      foreach ($donations as $donation) {
        ...
        if ($cityId) {
          $donation->update(['locality_id' => $cityId]);
          $updatedFallback++;
        }
        $bar->advance();
      }
      ...
      $bar->finish();
      $this->newLine();
      $this->info("Обновлено locality_id по региону: {$updatedFallback}");
    }
