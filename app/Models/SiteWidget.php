...
class SiteWidget extends Model
{
    ...
    switch ($this->widget_slug) {
      case 'hero':
        ...
        $config['hero_slides'] = $this->heroSlides->map(function ($slide) {
          return [
            ...
            'backgroundImage' => self::formatImageUrl($slide->background_image ?? ''),
      ...
      case 'gallery':
        $config['images'] = $this->galleryImages->map(function ($image) {
          return [
            'id' => $image->id,
            'url' => self::formatImageUrl($image->image_url ?? ''),
            'alt' => $image->alt_text,
      ...
      case 'image':
        if ($this->imageSettings) {
          $config = array_merge($config, [
            'image' => self::formatImageUrl($this->imageSettings->image_url ?? ''),
      ...
      case 'alumni_stats':
        // Форматируем иконки в колонках для фронтенда
        if (isset($config['columns']) && is_array($config['columns'])) {
          foreach ($config['columns'] as &$column) {
            if (isset($column['icon']) && is_string($column['icon']) && !empty($column['icon'])) {
              $column['icon'] = self::formatImageUrl($column['icon']);
            }
          }
  ...
  public function getFullRenderData(): array
  {
    ...
    switch ($this->widget_slug) {
      ...
      case 'teachers_slider':
        $orgId = $baseData['config']['organization_id'] ?? $this->site?->organization_id;
        if ($orgId) {
          $baseData['config']['organization_id'] = $orgId;
        }
        break;
    ...
    $baseData['sort_order'] = $this->sort_order;
  ...
  public function syncConfig(array $config): void
  {
    ...
    switch ($this->widget_slug) {
      ...
      case 'alumni_stats':
        // Для alumni_stats виджета нормализуем иконки в колонках перед сохранением
        if (isset($config['columns']) && is_array($config['columns'])) {
          foreach ($config['columns'] as &$column) {
            if (isset($column['icon']) && is_string($column['icon'])) {
              $column['icon'] = self::extractImagePathFromUrl($column['icon']);
            }
          }
          unset($column); // Сбрасываем ссылку
        }
  ...
  /**
   * Извлечь путь к файлу из полного URL (статический метод для использования везде)
   */
  public static function extractImagePathFromUrl(string $url): string
  {
    if (empty($url)) {
      return '';
    }
  ...
  /**
   * Форматировать URL изображения для возврата на фронтенд (добавляет /storage/ если нужно)
   */
    ...
    switch ($this->widget_slug) {
        ...
        if (isset($data['slides'])) {
          ...
          foreach ($data['slides'] as $index => $slideData) {
            ...
            try {
              ...
              $extractedPath = self::extractImagePathFromUrl($originalImageUrl);
          ...
          foreach ($data['slides'] as $index => $slideData) {
            try {
              ...
              $this->sliderSlides()->create([
                ...
                'overlay_gradient' => $slideData['overlayGradient'] ?? 'none',
                'overlay_gradient_intensity' => $slideData['overlayGradientIntensity'] ?? 50,
                'sort_order' => $slideData['order'] ?? $index + 1,
              ]);
            } catch (\Exception $e) {
              Log::error('Error creating slider slide', [
                'widget_id' => $this->id,
                'slide_index' => $index,
                'error' => $e->getMessage(),
                'slide_data' => $slideData,
        ...
        if (isset($data['images'])) {
          ...
          foreach ($data['images'] as $imageData) {
            ...
            $this->galleryImages()->create([
              'image_url' => self::extractImagePathFromUrl($imageUrl),
              'alt_text' => $imageData['alt'] ?? $imageData['alt_text'] ?? '',
              'caption' => $imageData['caption'] ?? '',
              'sort_order' => $imageData['order'] ?? $imageData['sort_order'] ?? 1,
            ]);
          }
        }
        break;
      ...
      case 'image':
        ...
        if (!empty($data)) {
          ...
          $imageSettingsData = [
            'image_url' => self::extractImagePathFromUrl($imageUrl),
            'alt_text' => $data['altText'] ?? $data['alt_text'] ?? '',
            ...
            'link_type' => $data['linkType'] ?? $data['link_type'] ?? 'internal',
            'open_in_new_tab' => $data['openInNewTab'] ?? $data['open_in_new_tab'] ?? false,
          ];
