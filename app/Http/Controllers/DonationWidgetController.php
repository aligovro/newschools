...
class DonationWidgetController extends Controller
{
  ...
  public function getWidgetData(Request $request, Organization $organization)
  {
    ...
    $data = [
      ...
      'payment_methods' => PaymentMethod::active()->ordered()->get()->map(function ($method) {
        return [
          'id' => $method->id,
          'name' => $method->name,
          'slug' => $method->slug,
    ...
    if ($projectId) {
      $project = OrganizationProject::where('organization_id', $organization->id)
  ...
  public function createDonation(Request $request, Organization $organization)
  {
    ...
    try {
      ...
      if (isset($data['project_id'])) {
        $project = OrganizationProject::where('id', $data['project_id'])
          ->where('organization_id', $organization->id)
          ->firstOrFail();
      }
  ...
  {
    $projects = OrganizationProject::where('organization_id', $organization->id)
      ->where('status', 'active')
      ->orderBy('created_at', 'desc')
      ->get()
