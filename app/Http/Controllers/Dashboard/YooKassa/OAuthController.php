...
class OAuthController extends Controller
{
  ...
  {
    // Проверяем, что callback URL доступен (для диагностики)
    Log::info('YooKassa OAuth callback endpoint accessed', [
      'method' => $request->method(),
      'url' => $request->fullUrl(),
      'path' => $request->path(),
      'callback_url_config' => config('services.yookassa_partner.callback_url'),
      'ip' => $request->ip(),
    ...
    try {
      ...
      // Если external_id был получен, обновляем его
      if ($externalId) {
