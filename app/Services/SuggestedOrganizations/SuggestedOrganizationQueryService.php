...
class SuggestedOrganizationQueryService
{
  ...
  public function baseQuery(): Builder
  {
    return SuggestedOrganization::query()
      ->with(['locality:id,name', 'reviewer:id,name,email'])
      ->select([
        'id',
        'name',
        'city_name',
  ...
  public function applyFilters(Builder $query, array $filters): Builder
  {
    ...
    if (!empty($filters['locality_id'])) {
      // Параметр называется locality_id для обратной совместимости фронта,
      // но фильтруем по locality_id как по основному справочнику.
