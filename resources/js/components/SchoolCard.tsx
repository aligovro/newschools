...
export default function SchoolCard({ school }: SchoolCardProps) {
    ...
    const schoolUrl = school.slug
        ? `/organization/${school.slug}`
        : `/organization/${school.id}`;

    // Формирование адреса: город + адрес
    const fullAddress = [school.city?.name, school.address]
        .filter(Boolean)
        .join(', ');

    const targetAmount = school.donations_total || 0;
    const collectedAmount = school.donations_collected || 0;
    const progressPercentage =
        targetAmount > 0
            ? Math.round((collectedAmount / targetAmount) * 100)
            : 0;

    const directorName = school.director?.name || school.director_name || null;
                    ...
                    <div className="mb-4 grid grid-cols-3 gap-4 border-t border-gray-200 pt-4">
                        <div className="text-center">
                            ...
                            <div className="text-lg font-bold text-gray-900">
                                {formatNumber(school.members_count || 0)}
                            </div>
                        </div>
                        <div className="text-center">
                            <div className="mb-1 flex items-center justify-center text-gray-400">
                            ...
                            <div className="text-lg font-bold text-gray-900">
                                {formatNumber(school.sponsors_count || 0)}
                            </div>
                        </div>
                        <div className="text-center">
                            <div className="mb-1 flex items-center justify-center text-gray-400">
                            ...
                            <div className="text-lg font-bold text-gray-900">
                                {formatNumber(school.projects_count || 0)}
                            </div>
                        </div>
                    </div>
