    ...
    const handleImageUpload = useCallback(
        async (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (!acceptedTypes.includes(fileType) && !isSvg) {
                alert(
                    `Неподдерживаемый тип файла. Допустимые: jpeg, jpg, png, gif, webp, svg`,
                );
            ...
            try {
                ...
                // Создаём обёртку (span.rte-image) для кнопки редактирования
                ...
                img.onload = () => {
                    ...
                    // Добавляем кнопку редактирования для всех изображений
                    initializeImageResize(img);

                    // Обработчик клика — выделяем изображение
                    ...
                    // Двойной клик — открываем диалог редактирования
