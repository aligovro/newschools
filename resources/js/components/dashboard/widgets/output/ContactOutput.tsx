...
import { ContactOutputConfig, WidgetOutputProps } from './types';

export const ContactOutput: React.FC<WidgetOutputProps> = ({
    widget,
    className,
    style,
}) => {
    ...
    const hasAnyContact =
        phone || email || address || website || Object.keys(social).length > 0;

    if (!hasAnyContact) {
        return (
            <div
                className={`contact-output contact-output--empty ${className || ''}`}
                style={style}
            >
                <div className="flex h-32 items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50">
                    <span className="text-gray-500">
                        Контактная информация не настроена
                    </span>
                </div>
            </div>
        );
