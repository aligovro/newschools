...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (Schema::hasTable('localities')) {
            // Карта городов: city_id -> locality_id (по региону+названию)
            $cityMap = [];
            if (Schema::hasTable('cities')) {
                $rows = DB::table('cities')
                    ->join('localities', function ($join) {
                        $join->on('cities.region_id', '=', 'localities.region_id')
                            ->on('cities.name', '=', 'localities.name');
                    })
                    ->select('cities.id as city_id', 'localities.id as locality_id')
                    ->get();

                foreach ($rows as $row) {
                    $cityMap[(int) $row->city_id] = (int) $row->locality_id;
                }
            }
            ...
            if (Schema::hasTable('organizations')) {
                $orgs = DB::table('organizations')
                    ->select('id', 'city_id', 'settlement_id')
                ...
                foreach ($orgs as $org) {
                    ...
                    } elseif ($org->city_id && isset($cityMap[$org->city_id])) {
                        $localityId = $cityMap[$org->city_id];
                    }
            ...
            if (Schema::hasTable('suggested_organizations') && !empty($cityMap)) {
                ...
                foreach ($items as $item) {
                    $localityId = $cityMap[$item->city_id] ?? null;
                    if ($localityId) {
